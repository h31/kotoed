...
import org.jetbrains.research.kotoed.database.tables.Tag
...
class ProjectVerticle : AbstractKotoedVerticle(), Loggable {
    ...
    suspend fun handleSearchForCourse(query: SearchQueryWithTags): JsonArray {
        ...
        val subQueryClosedSubmissions2 = query(Tables.SUBMISSION) {
            find {
                state = SubmissionState.closed
            }

            join(Tables.SUBMISSION_TAG) {
                join(Tables.TAG) {
                    find(TagRecord().apply { name = "permanent" })
                }
            }
        }
        ...
        val q = ComplexDatabaseQuery(tableName)
                ...
                .rjoin(subQueryClosedSubmissions2, "project_id", "closedSubmissions")
        ...
        val reqWithVerificationData = if (query.withVerificationData ?: false) {
            projects.map { json ->
                ...
                val tags = json.getJsonArray("closedSubmissions")
                        ?.filterIsInstance<JsonObject>()
                        ?.map {
                            val list = mutableListOf<String>()
                            val tagRecords = it.safeNav("submission_tags") as? JsonArray
                            for (record in tagRecords ?: JsonArray()) {
                                val tag = (record as JsonObject).safeNav("tag") as JsonObject
                                val name = tag.safeNav("name").toString()
                                list += name
                            }
                            if ("permanent" !in list) {
                                return@map 0
                            }
                            val penalty = list.map { it.toIntOrNull() }.filterNotNull().sum()
                            penalty
                        }
                json["permanentPenalty"] = (tags ?: listOf()).sum()
                json
            }
        } else projects
